...
describe("UserApplicationService", () => {
  ...
  it("createApplication validates, uploads files, and creates app", async () => {
    ...
    const files: any = { coverLetterFile: { originalname: "c.txt" }, resumeFile: { originalname: "r.pdf" } };
    ...
    await expect(svc.createApplication(1, 999, {} as any)).rejects.toThrow("Job not found");

    // user not found
    mockPrisma.job.findUnique.mockResolvedValueOnce({ id: 2 });
    mockPrisma.user.findUnique.mockResolvedValueOnce(null);
    await expect(svc.createApplication(1, 2, {} as any)).rejects.toThrow("User not found");

    // duplicate application
    mockPrisma.job.findUnique.mockResolvedValueOnce({ id: 2 });
    mockPrisma.user.findUnique.mockResolvedValueOnce({ id: 1 });
    mockPrisma.application.findFirst.mockResolvedValueOnce({ id: 99 });
    await expect(svc.createApplication(1, 2, {} as any)).rejects.toThrow("already applied");
  });
  ...
  it("deleteApplication enforces ownership and deletes", async () => {
    ...
    await expect(svc.deleteApplication(1, 1)).rejects.toThrow("Application not found");

    // forbidden
    mockPrisma.application.findUnique.mockResolvedValueOnce({ id: 5, applicantId: 2 });
    await expect(svc.deleteApplication(5, 1)).rejects.toThrow("permission");

    // ok
    mockPrisma.application.findUnique.mockResolvedValueOnce({ id: 5, applicantId: 1 });
    mockPrisma.application.delete.mockResolvedValueOnce({});
    await svc.deleteApplication(5, 1);
    expect(mockPrisma.application.delete).toHaveBeenCalledWith({ where: { id: 5 } });
  });
});
