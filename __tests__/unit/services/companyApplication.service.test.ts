...
describe('CompanyApplicationService', () => {
  ...
  it('getApplicationById maps fields and enforces ownership', async () => {
    ...
    expect(result).toEqual(
      expect.objectContaining({
        ...
        applicant: expect.objectContaining({
          experience: [
            expect.objectContaining({
              startDate: expect.stringMatching(/^2020-01-01/),
              endDate: null,
            }),
          ],
          education: [
            expect.objectContaining({
              startDate: expect.stringMatching(/^2016-01-01/),
              endDate: expect.stringMatching(/^2020-01-01/),
            }),
          ],
        }),
      }),
    ...
    await expect(svc.getApplicationById(7, 999)).rejects.toThrow(
  ...
  it('updateApplicationStatus updates, sends email, and returns updated app', async () => {
    ...
    expect(emailMock.sendApplicationStatusUpdate).toHaveBeenCalledWith(
      updated,
      'PROCESSING',
    );
    ...
    mockPrisma.application.findUnique.mockResolvedValueOnce(null);
    await expect(
      svc.updateApplicationStatus(4, 3, 'REJECTED' as any),
    ...
    await expect(
      svc.updateApplicationStatus(4, 1, 'PROCESSING' as any),
    ...
    (emailMock.sendApplicationStatusUpdate as jest.Mock).mockRejectedValueOnce(
      new Error('smtp fail'),
    );
    await expect(
      svc.updateApplicationStatus(4, 3, 'PROCESSING' as any),
