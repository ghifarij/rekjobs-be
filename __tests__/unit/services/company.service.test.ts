...
jest.mock("bcryptjs", () => ({ __esModule: true, hash: jest.fn(async (pw: string) => `hashed_${pw}`) }));
...
describe("CompanyService", () => {
  ...
  it("getProfile returns selected fields", async () => {
    ...
    expect(mockPrisma.company.findUnique).toHaveBeenCalledWith(expect.objectContaining({ where: { id: 1 } }));
  ...
  it("updateProfile hashes password and updates company", async () => {
    ...
    const res = await svc.updateProfile(1, { name: "Acme", password: "new" } as any);

    expect(mockPrisma.company.update).toHaveBeenCalledWith(expect.objectContaining({
      where: { id: 1 },
      data: expect.objectContaining({ name: "Acme", password: expect.stringMatching(/^hashed_/)}),
    }));
