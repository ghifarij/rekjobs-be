...
describe("CompanyInterviewService", () => {
  ...
  it("createInterview checks application and creates interview", async () => {
    ...
    await expect(svc.createInterview(999, new Date(), undefined)).rejects.toThrow("Application not found");
  });
  ...
  it("rescheduleInterview enforces ownership and updates", async () => {
    ...
    await expect(svc.rescheduleInterview(1, 2, new Date())).rejects.toThrow("Interview not found");

    // forbidden
    mockPrisma.interview.findUnique.mockResolvedValueOnce({ id: 1, application: { job: { companyId: 10 } } });
    await expect(svc.rescheduleInterview(1, 2, new Date())).rejects.toThrow("Forbidden");

    // ok
    mockPrisma.interview.findUnique.mockResolvedValueOnce({ id: 1, application: { job: { companyId: 2 } } });
    ...
    expect(mockPrisma.interview.update).toHaveBeenCalledWith(expect.objectContaining({
      where: { id: 1 },
