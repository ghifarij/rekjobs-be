...
jest.mock("bcrypt", () => ({
  __esModule: true,
  default: {
    compare: jest.fn(async (pw: string, hash: string) => pw === "valid" && !!hash),
...
const jwt = {
  ...
  verify: jest.fn((token: string) => ({ email: "foo@bar.com", id: 1 })),
};
jest.mock("jsonwebtoken", () => ({ __esModule: true, default: jwt, sign: jwt.sign, verify: jwt.verify }));
...
jest.mock("../../../src/utils/email", () => ({ __esModule: true, ...emailUtils }));
...
describe("AuthCompanyService", () => {
  ...
  beforeEach(() => {
    jest.clearAllMocks();
    process.env = { ...originalEnv, JWT_KEY: "secret", NEXT_PUBLIC_BASE_URL_FE: "https://fe" };
  });
  afterAll(() => { process.env = originalEnv; });

  it("loginCompany returns token on valid credentials", async () => {
    mockPrisma.company.findUnique.mockResolvedValue({ id: 3, email: "e@c.com", name: "C", password: "hash", logo: "l" });
    const svc = new AuthCompanyService();
    const res = await svc.loginCompany("e@c.com", "valid");
    expect(res.token).toBe("jwt-token");
    expect(res.company).toEqual({ id: 3, email: "e@c.com", name: "C", logo: "l" });
  });
  ...
  it("registerCompany creates pending company and sends verification", async () => {
    ...
    expect(mockPrisma.company.create).toHaveBeenCalledWith(expect.objectContaining({
      data: expect.objectContaining({ email: "c@x.com", verificationToken: expect.any(String), isVerified: false }),
    }));
    expect(emailUtils.sendCompanyVerificationEmail).toHaveBeenCalledWith("c@x.com", expect.any(String));
  ...
  it("verifyCompany updates data and clears token", async () => {
    ...
    mockPrisma.company.findFirst.mockResolvedValue({ id: 10, verificationToken: "tok", isVerified: false });
    mockPrisma.company.update.mockResolvedValue({});
    const svc = new AuthCompanyService();
    const res = await svc.verifyCompany("tok", { name: "Acme", password: "newpass" } as any);
    expect(mockPrisma.company.update).toHaveBeenCalledWith(expect.objectContaining({
      where: { id: 10 },
      data: expect.objectContaining({ name: "Acme", password: expect.stringMatching(/^hashed_/), isVerified: true, verificationToken: null }),
    }));
    expect(res).toEqual({ message: "Account verified successfully" });
  });

  it("forgotPasswordCompany sends reset email with link", async () => {
    mockPrisma.company.findUnique.mockResolvedValue({ id: 8, email: "z@c.com" });
    const svc = new AuthCompanyService();
    ...
    expect(emailUtils.sendEmail).toHaveBeenCalledWith(expect.objectContaining({
      ...
      html: expect.stringContaining("https://fe/auth/company/reset-password?token="),
    }));
  ...
  it("resetPasswordCompany hashes and updates password", async () => {
    ...
    mockPrisma.company.findUnique.mockResolvedValue({ id: 77, resetPasswordToken: token });
    mockPrisma.company.update.mockResolvedValue({});
    const svc = new AuthCompanyService();
    const res = await svc.resetPasswordCompany(token, "newpw");
    expect(mockPrisma.company.update).toHaveBeenCalledWith(expect.objectContaining({
      where: { id: 77 },
      data: expect.objectContaining({ password: expect.stringMatching(/^hashed_/), resetPasswordToken: null, resetPasswordExpires: null }),
    }));
  ...
  it("checkVerificationStatus returns verification flag", async () => {
    jwt.verify.mockReturnValueOnce({ email: "v@co.com" } as any);
    mockPrisma.company.findUnique.mockResolvedValue({ email: "v@co.com", isVerified: true });
    const svc = new AuthCompanyService();
  ...
  describe("socialLogin", () => {
    it("throws when googleId missing", async () => {
      const svc = new AuthCompanyService();
      await expect(svc.socialLogin("" as any)).rejects.toThrow("Google ID is required");
    });

    it("throws when email registered without googleId", async () => {
      // first findFirst by email returns existing without googleId
      mockPrisma.company.findFirst.mockResolvedValueOnce({ id: 1, email: "a@c.com", googleId: null });
      const svc = new AuthCompanyService();
    ...
    it("throws when email registered with different googleId", async () => {
      mockPrisma.company.findFirst.mockResolvedValueOnce({ id: 2, email: "b@c.com", googleId: "other" });
      const svc = new AuthCompanyService();
    ...
    it("creates new company when not found by googleId", async () => {
      ...
      const svc = new AuthCompanyService();
      const out = await svc.socialLogin("gid-abcdef", "new@c.com", undefined, "pic");
      expect(mockPrisma.company.create).toHaveBeenCalled();
      expect(out.token).toBe("jwt-token");
      expect(out.company).toEqual({ id: 11, email: "new@c.com", name: "Google Company gid-abc", logo: "pic" });
    ...
    it("updates existing company info when found by googleId", async () => {
      ...
      // 2) found by googleId
      mockPrisma.company.findFirst.mockResolvedValueOnce({ id: 21, email: "old@c.com", name: "Old", logo: null, googleId: "gid-21" });
      mockPrisma.company.update.mockResolvedValue({ id: 21, email: "new@c.com", name: "New", logo: "pic" });
      const svc = new AuthCompanyService();
      const out = await svc.socialLogin("gid-21", "new@c.com", "New", "pic");
      expect(mockPrisma.company.update).toHaveBeenCalledWith(expect.objectContaining({
        where: { id: 21 },
        data: expect.objectContaining({ email: "new@c.com", name: "New", logo: "pic" }),
      }));
      expect(out.company).toEqual({ id: 21, email: "new@c.com", name: "New", logo: "pic" });
    });
  });
  ...
  describe("loginCompany negatives", () => {
    it("throws when company not found", async () => {
      mockPrisma.company.findUnique.mockResolvedValue(null);
      const svc = new AuthCompanyService();
      await expect(svc.loginCompany("x@y.com", "valid")).rejects.toThrow("Company not found");
    });

    it("throws on invalid password", async () => {
      mockPrisma.company.findUnique.mockResolvedValue({ id: 9, email: "e@c.com", name: "C", password: "hash", logo: null });
      const svc = new AuthCompanyService();
      await expect(svc.loginCompany("e@c.com", "wrong"))
