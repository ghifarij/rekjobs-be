...
describe("AuthUserService", () => {
  ...
  it("loginUser returns token on valid credentials", async () => {
    mockPrisma.user.findUnique.mockResolvedValue({ id: 3, email: "e@u.com", name: "U", password: "hash" });
  ...
  it("registerUser creates pending user and sends verification", async () => {
    ...
    expect(mockPrisma.user.create).toHaveBeenCalledWith(expect.objectContaining({
      data: expect.objectContaining({ email: "u@x.com", verificationToken: expect.any(String), isVerified: false }),
    }));
    expect(emailUtils.sendUserVerificationEmail).toHaveBeenCalledWith("u@x.com", expect.any(String));
  ...
  it("verifyUser updates data and sets verified", async () => {
    ...
    const svc = new AuthUserService();
    const res = await svc.verifyUser("tok", { username: "Bob", password: "new", no_handphone: "123" });
    expect(mockPrisma.user.update).toHaveBeenCalledWith(expect.objectContaining({
      where: { id: 10 },
      data: expect.objectContaining({ name: "Bob", phone: "123", password: expect.stringMatching(/^hashed_/), isVerified: true }),
    }));
  ...
  it("forgotPassword sends reset email with link", async () => {
    ...
    expect(emailUtils.sendEmail).toHaveBeenCalledWith(expect.objectContaining({
      ...
      html: expect.stringContaining("https://fe/auth/user/reset-password?token="),
    }));
  ...
  it("resetPassword hashes and updates password", async () => {
    ...
    mockPrisma.user.findUnique.mockResolvedValue({ id: 77, resetPasswordToken: token });
    mockPrisma.user.update.mockResolvedValue({});
    const svc = new AuthUserService();
  ...
  it("checkVerificationStatus returns verification flag", async () => {
    jwt.verify.mockReturnValueOnce({ email: "v@u.com" } as any);
    mockPrisma.user.findUnique.mockResolvedValue({ email: "v@u.com", isVerified: true });
    const svc = new AuthUserService();
  ...
  describe("socialLogin", () => {
    ...
    it("throws when email registered without googleId", async () => {
      mockPrisma.user.findFirst.mockResolvedValueOnce({ id: 1, email: "a@u.com", googleId: null });
      const svc = new AuthUserService();
    ...
    it("throws when email registered with different googleId", async () => {
      mockPrisma.user.findFirst.mockResolvedValueOnce({ id: 2, email: "b@u.com", googleId: "other" });
      const svc = new AuthUserService();
    ...
    it("creates new user when not found by googleId", async () => {
      ...
      mockPrisma.user.create.mockResolvedValue({ id: 11, email: "new@u.com", name: "Google User gid-abc", avatar: "pic" });
      const svc = new AuthUserService();
      const out = await svc.socialLogin("gid-abcdef", "new@u.com", undefined, "pic");
      expect(mockPrisma.user.create).toHaveBeenCalled();
      expect(out.token).toBe("jwt-token");
      expect(out.user).toEqual({ id: 11, email: "new@u.com", name: "Google User gid-abc", avatar: "pic" });
    ...
    it("updates existing user info when found by googleId", async () => {
      ...
      mockPrisma.user.findFirst.mockResolvedValueOnce({ id: 21, email: "old@u.com", name: "Old", avatar: null, googleId: "gid-21" });
      mockPrisma.user.update.mockResolvedValue({ id: 21, email: "new@u.com", name: "New", avatar: "pic" });
      const svc = new AuthUserService();
      const out = await svc.socialLogin("gid-21", "new@u.com", "New", "pic");
      expect(mockPrisma.user.update).toHaveBeenCalledWith(expect.objectContaining({
        where: { id: 21 },
        data: expect.objectContaining({ email: "new@u.com", name: "New", avatar: "pic" }),
      }));
      expect(out.user).toEqual({ id: 21, email: "new@u.com", name: "New", avatar: "pic" });
    });
  });
  ...
  describe("loginUser negatives", () => {
    it("throws when user not found", async () => {
      mockPrisma.user.findUnique.mockResolvedValue(null);
      const svc = new AuthUserService();
      await expect(svc.loginUser("x@y.com", "valid")).rejects.toThrow("User not found");
    });

    it("throws on invalid password", async () => {
      mockPrisma.user.findUnique.mockResolvedValue({ id: 9, email: "e@u.com", name: "U", password: "hash" });
      const svc = new AuthUserService();
      await expect(svc.loginUser("e@u.com", "wrong"))
